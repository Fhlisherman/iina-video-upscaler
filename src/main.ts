...
function updateMenu(): void {
  menu.clear();
  menu.addItem(menu.item("Enable GPU Upscale (FSRCNNX)", () => applyShader("upscale"), {
    selected: currentMode === "upscale"
  }));
  menu.addItem(menu.item("Enable GPU Downscale (SSim)", () => applyShader("downscale"), {
    selected: currentMode === "downscale"
  }));
  menu.addItem(menu.separator());
  menu.addItem(menu.item("Disable GPU Effects", () => {
    mpv.command("glsl-shaders-clr");
    currentMode = "none";
    updateMenu();
  }));
